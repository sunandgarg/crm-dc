...
async function main() {
  ...
  const admissionsTeam = await prisma.crm_teams.upsert({
    where: { name: 'Admissions' },
    create: { name: 'Admissions', description: 'Primary admissions counselling team', manager_id: admin.id },
    update: { is_active: true, manager_id: admin.id },
  });
  await prisma.appUser.update({ where: { id: admin.id }, data: { team_id: admissionsTeam.id, team: admissionsTeam.name } });

  const stageDefinitions = [
    ['Untouched', '#e11d48'], ['Attempted', '#f59e0b'], ['Contacted', '#0891b2'],
    ['Follow-up', '#7c3aed'], ['Interested', '#0d9488'], ['Application Started', '#4f46e5'],
    ['Application Submitted', '#9333ea'], ['Enrolled', '#16a34a'], ['Not Interested', '#64748b'], ['Lost', '#dc2626'],
  ];
  await prisma.pipeline_stages.updateMany({ data: { is_default: false } });
  const stages = [];
  for (let index = 0; index < stageDefinitions.length; index += 1) {
    const [name, color] = stageDefinitions[index];
    stages.push(await prisma.pipeline_stages.upsert({ where: { name }, create: { name, color, sort_order: index, is_default: index === 0 }, update: { color, sort_order: index, is_default: index === 0 } }));
  }
  ...
  const contacts = [
    { id: '21111111-1111-4111-8111-111111111111', name: 'Aarav Sharma', email: 'aarav@example.com', mobile: '9876500001', source: 'Google Ads', course: 'MBA', city: 'Delhi', priority: 'High', lead_score: 86, stage_id: stages[3].id, owner_id: admin.id, assigned_to: admin.full_name },
    { id: '21111111-1111-4111-8111-111111111112', name: 'Meera Iyer', email: 'meera@example.com', mobile: '9876500002', source: 'Website', course: 'B.Tech', city: 'Bengaluru', priority: 'Medium', lead_score: 72, stage_id: stages[5].id, owner_id: admin.id, assigned_to: admin.full_name },
  ...
  await prisma.crm_tasks.upsert({
    ...
    create: { id: '41111111-1111-4111-8111-111111111111', contact_id: contacts[0].id, title: 'Collect graduation marksheet', priority: 'high', assigned_to: admin.full_name, assignee_id: admin.id, due_at: new Date(Date.now() + 86_400_000) },
    update: {},
  });
