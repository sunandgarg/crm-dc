...
export async function inspectObject(objectKey: string) {
  const result = await objectStorage.send(new HeadObjectCommand({ Bucket: config.STORAGE_BUCKET, Key: objectKey }));
  return { contentType: result.ContentType || null, sizeBytes: result.ContentLength ?? null, etag: result.ETag?.replaceAll('"', '') || null };
}
